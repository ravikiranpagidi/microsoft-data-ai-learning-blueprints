...
# MAGIC %md
# MAGIC # 01 - Bronze Ingestion: Web Activity
# MAGIC
# MAGIC Ingests JSON Lines web activity into Bronze Delta.
...
raw_df = spark.read.json(f"{raw_base_path}/web_activity")
bronze_df = (
...
bronze_df.write.format("delta").mode("append").option("mergeSchema", "true").saveAsTable(f"{catalog}.bronze.brz_web_activity")
print(f"Wrote {row_count:,} web activity rows")
