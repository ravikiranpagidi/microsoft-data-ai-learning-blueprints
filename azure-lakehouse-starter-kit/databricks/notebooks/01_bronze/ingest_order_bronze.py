...
# MAGIC %md
# MAGIC # 01 - Bronze Ingestion: Orders And Order Items
# MAGIC
# MAGIC Ingests order headers and order line items into separate Bronze Delta tables.
...
def ingest_csv(dataset_name: str, target_table: str) -> None:
    source_path = f"{raw_base_path}/{dataset_name}"
    print(f"Reading {dataset_name} from {source_path}")
...
ingest_csv("orders", f"{catalog}.bronze.brz_orders")
ingest_csv("order_items", f"{catalog}.bronze.brz_order_items")
